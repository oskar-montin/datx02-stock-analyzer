...
public class TestBot5 {
	...
	public TestBot5(int startDate, int testLength) {
	...
	private void init() {
		...
		this.maxPrice = new double[stocks.length];
		i=0;
		for(Stock s:stocks) {
	...
	public void run() {
		...
		while(currentDate<endDate-startDate) {
			System.out.println("Index: "+(currentDate-50));
	...
	private ArrayList<Transaction> portfolioConstructor(double[] stockValues) {
		...
		for(int i = 0;i<stockValues.length;i++) {
			...
			if(bestBuyValues[i]!=0) {
				maxPrice[i] = Math.max(maxPrice[i], data[i].get(currentDate).getClosePrice());
			}
			if(stockValues[i]>0) {
				...
				if((bestBuyValues[i]<=Math.abs(stockValues[i]))||
						(data[i].get(currentDate).getClosePrice()<maxPrice[i])) {
					today = data[i].get(currentDate);
					//today = data[i].get(currentDate);
				...
				if(bestBuyValues[i]>0) {
					TrendLine tl = new TrendLine(new PriorityQueue<DailyData>(data[i].subList(0, currentDate)), 15);
					if(tl.getSignal()==Signal.SELL) {
						today = data[i].get(currentDate);
	...
	private TreeMap<SimpleData,Integer> distribute(TreeMap<SimpleData,Double> buyCandidates, double balance, int maxCount) {
		...
		TreeMap<SimpleData,Double> newValues = new TreeMap<SimpleData,Double>();
		...
		for(Entry<SimpleData,Double> entry:choosenQueue) {
			...
			newValues.put(entry.getKey(), currentShare);
		...
		if(stocksOwned<3) {
			balance = balance*stocksOwned/3;
		}
		
		//Calculate shares and add transactions
		for(Entry<SimpleData,Double> e: choosenEntries.entrySet()) {
			double eVal = e.getValue();
			
			double share = balance*eVal/sum;
			int amount = (int) (share/e.getKey().getValue());
			
			for(Entry<SimpleData,Double> neVal:newValues.entrySet()) {
				if(neVal.getKey().getStock().compareTo(e.getKey().getStock())==0) {
					if((neVal.getValue()+amount*e.getKey().getValue())/
							getTotalAssets(user)<0.5) {
						distribution.put(e.getKey(), amount);
						usedBalance += amount*e.getKey().getValue();
					} else {
						share = getTotalAssets(user)*(0.5-neVal.getValue());
						amount = (int) (share/e.getKey().getValue());
						if(amount>0) {
							distribution.put(e.getKey(), amount);
							usedBalance += amount*e.getKey().getValue();
						}
	...
	public static void main(String[] args) {
		...
		//TestBot4 bot = new TestBot4(45, 338);
		TestBot5 bot = new TestBot5(45, 540);
		//TestBot5 bot = new TestBot5(383, 200);
		bot.run();
		//bot.printEValues();
