...
public class MACDValueSO2 implements AnalysisMethod {
	...
	public MACDValueSO2(Collection<DailyData> queue, 
	...
	public Signal getSignal() {
		if(macd.getMACDFiltered().peek().getValue() < 0 && so.getSignal() == Signal.SELL)
			return Signal.SELL;
		else if(macd.getMACDFiltered().peek().getValue() > 0 && so.getSignal() == Signal.BUY)
			return Signal.BUY;
		else
			return Signal.NONE;
	}
